...
SELECT *  FROM XBITBLSV.DimEmployee  WHERE HrNum IN

(SELECT Lvl1MgmtId FROM  XBITBLSV.DimEmployee AS A

WHERE  A.HrNum IN (SELECT * FROM DLBIFO.CK_UNION_TEC) AND IsCurrFl=1 GROUP BY  Lvl1MgmtId HAVING COUNT(Lvl1MgmtId )>1
